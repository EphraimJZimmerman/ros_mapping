...
class ImuFollower:
    def __init__(self):
        ...
        self.bearing_sub = rospy.Subscriber(
            '/robot/bearing', Float64, self.bearing_cb)
        self.turn_angle_sub = rospy.Subscriber(
            '/robot/turn_angle', Float64, self.turn_angle_cb)
        self.imu_mag_sub = rospy.Subscriber(
            'imu/mag_corrected', MagneticField, self.imu_cb)
        self.cur_target_yaw = None
        # angles should never be negative so functions as a non-initialized value
        self.prev_target_yaw = -1
        # self.odom_sub = rospy.Subscriber('odom', Odometry, self.odom_cb)

    def bearing_cb(self, msg):
        print("callback: " + str(msg.data))
        self.cur_target_yaw = self.convertNegativeAngles(
            msg.data * (math.pi/180))
    ...
    def turn_to_heading(self, target_yaw, base_vel):
        ...
        # a 0-rotation twist to stop movement
        self.cmd_vel_pub.publish(Twist())
