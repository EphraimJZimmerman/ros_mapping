...
def plot_dist_from_origin(df, title, xlabel, ylabel, save_img=False):
    ...
    # print(alpha_xy, alpha_xz, alpha_yz)

    # Scatter plot using calculated distances for color and alpha
    plt.scatter(df.iloc[:, 0], df.iloc[:, 1], s=2,
                c=distances_xy, cmap=c_map, alpha=0.7, label='XY')
    plt.scatter(df.iloc[:, 0], df.iloc[:, 2], s=2,
                c=distances_xz, cmap=c_map, alpha=0.7, label='XZ')
    plt.scatter(df.iloc[:, 1], df.iloc[:, 2], s=2,
                c=distances_yz, cmap=c_map, alpha=0.7, label='YZ')
...
def plot_comparison(original_df, comparated_df, title, save_img=False):
    s_size = 2
    plt.figure(title, figsize=figure_size)
    plt.scatter(original_df['x'], original_df['y'], s=s_size,
                label='Original XY', color='lightblue', alpha=0.7)
    plt.scatter(original_df['x'], original_df['z'], s=s_size,
                label='Original XZ', color='skyblue', alpha=0.7)
    plt.scatter(original_df['y'], original_df['z'], s=s_size,
                label='Original YZ', color='deepskyblue', alpha=0.7)
    plt.scatter(comparated_df['scaled_corrected_x'], comparated_df['scaled_corrected_y'],
                s=s_size, label='Comparated XY', color='lightcoral', alpha=0.7)
    plt.scatter(comparated_df['scaled_corrected_x'], comparated_df['scaled_corrected_z'],
                s=s_size, label='Comparated XZ', color='hotpink', alpha=0.7)
    plt.scatter(comparated_df['scaled_corrected_y'], comparated_df['scaled_corrected_z'],
                s=s_size, label='Comparated YZ', color='orchid', alpha=0.7)
    plt.title(title)
    plt.xlabel("XY")
    plt.ylabel("YZ")
    plt.legend()
...
def main():
    ...
    # helper function to load and clean the data, makes sure only lines with x,y,z values are present
    original_df = load_clean_csv(DATA_FILE)
    ...
    offset_x, offset_y, offset_z, filtered_offset_df = apply_offset_correction(
        filtered_df)
    print(
        f"offset values X {int(offset_x)}\tY {int(offset_y)}\tZ {int(offset_z)} \txyz({int(offset_x)},{int(offset_y)},{int(offset_z)})")

    # Step 4: Apply scale correction and plot the data
    scale_x, scale_y, scale_z, filtered_offset_scale_df = apply_scale_correction(
        filtered_offset_df)
    print(
        f"scale values X {scale_x:.6f}\tY {scale_y:.6f}\tZ {scale_z:.6f} \txyz({scale_x:.6f},{scale_y:.6f},{scale_z:.6f})")
    ...
    plot_data(original_df, title="Figure 1 - Data with outliers",
              xlabel="XY", ylabel="YZ", save_img=save_plots_to_file)
    plot_dist_from_origin(original_df, title="Figure 1.1 - DistofOrigin of Data with outliers",
                          xlabel="XY", ylabel="YZ", save_img=save_plots_to_file)

    # Step 7: Plot some information about the outlier cleaned data
    plot_data(filtered_df.sample(frac=plot_sampling_percentage),
              title="Figure 2 - Data without outliers", xlabel="XY", ylabel="YZ", save_img=save_plots_to_file)
    plot_dist_from_origin(filtered_df.sample(frac=plot_sampling_percentage),
                          title="Figure 2.2 - DistofOrigin of Data without outliers", xlabel="XY", ylabel="YZ", save_img=save_plots_to_file)

    # Step 8: Plot some information about the offset corrected data
    plot_data(filtered_offset_df.sample(frac=plot_sampling_percentage), title="Figure 3 - Clean data and offset",
              xlabel="Offset corrected XY", ylabel="Offset corrected YZ", save_img=save_plots_to_file)
    plot_dist_from_origin(filtered_offset_df.sample(frac=plot_sampling_percentage), title="Figure 3.1 - DistofOrigin of Clean data and offset",
                          xlabel="Offset corrected XY", ylabel="Offset corrected YZ", save_img=save_plots_to_file)

    # Step 9: Plot some information about the offset and scale corrected data
    plot_data(filtered_offset_scale_df.sample(frac=plot_sampling_percentage), title="Figure 4 - Clean data offset and scale",
              xlabel="Offset+Scale Corrected XY", ylabel="Offset+Scale Corrected YZ", save_img=save_plots_to_file)
    plot_dist_from_origin(filtered_offset_scale_df.sample(frac=plot_sampling_percentage), title="Figure 4.1 - DistofOrigin of Clean data offset and scale",
                          xlabel="Offset+Scale Corrected XY", ylabel="Offset+Scale Corrected YZ", save_img=save_plots_to_file)

    # Step 5: Show a comparison of the original and calibrated plots. make sure the calibrated plot is centered around 0.
    plot_comparison(filtered_df.sample(frac=plot_sampling_percentage), filtered_offset_scale_df.sample(
        frac=plot_sampling_percentage), "Figure 5 - Comparison original vs calibrated", save_img=save_plots_to_file)
